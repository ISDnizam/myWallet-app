...
class IncomingScreen extends React.Component {
  static navigationOptions = ({ navigation }) => ({
    title: translate('Incoming Transaction'),
  ...
  getTx(date) {
    return fetch(GlobalSetting.url_api + '/transaksi/all?type=in&date='+date+'&id_user=' + this.state.id_user)
      ...
      <View>
        ...
        <View style={styles.cardProfileHeader} >
          <View style={{position: 'absolute'}}>
            <Text  style={{marginBottom: 75,marginLeft: 10,marginTop: 10,color:'#3b3a3a',fontSize:12, fontWeight:'bold'}}>
        ...
        <View style={styles.cardHome}>
          <View style={{flex:1, flexDirectio:'row'}}>
            <View style={{position: 'relative'}}>
              <Text  style={{marginBottom: 5,marginLeft: 0,marginTop: 0,color:'#3b3a3a',fontSize:12, fontWeight:'bold'}}>
    ...
    return (
      <View style={[styles.container]}>
        ...
        <Tabs selected={this.state.type} style={{backgroundColor:'white', elevation:2}}
          ...
          <Text name="TOTAL" selectedIconStyle={{borderTopWidth:2,borderTopColor:mainColor()}}  style={[styles.textGray, styles.boldFont]}>{translate('All Income')}</Text>
        </Tabs>
        {this.renderContent()}
      </View>
    )
